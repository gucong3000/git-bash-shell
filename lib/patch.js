...
function getPath (options) {
	...
	return envPath && envPath.split(';').filter(Boolean).map(getEnv);
}
...
function fixFilePath (options) {
	...
	function resolve (file) {
		return path.win32.resolve(pwd, file || options.file);
	}
	...
	if (/^~\//.test(options.file)) {
		// 处理linux风格的home路径
		setFile(findFile([pathJoin(osHomedir(), options.file.slice(2))]));
	} else if (/^\//.test(options.file) || /^(\\).+?\1(?:ba|da|z)?sh(?:\.exe)?$/.test(options.file)) {
		// 处理linux风格的绝对路径
		setFile(findFile([pathJoin(gitPath, options.file.slice(1))]));
	} else if (/^[^\\/.]+$/.test(options.file)) {
		// 处理无路径, 无扩展名的命令名称
		var envPath = getPath(options).map(function (dir) {
			return resolve(pathJoin(dir, options.file));
		});
...
var newPath = [
	...
	return pathJoin(gitPath, subDir);
});
var delimiter = ';';
